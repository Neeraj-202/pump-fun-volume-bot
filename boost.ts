...
const USE_WALLET_LIST = retrieveEnvVariable('USE_WALLET_LIST', logger) === 'true';
const WALLET_LIST_REFRESH_INTERVAL = Number(retrieveEnvVariable('WALLET_LIST_REFRESH_INTERVAL', logger));
...
async function init(): Promise<void> {
  ...
  logger.info(`Wallet list: ${USE_WALLET_LIST}`);
  logger.info(`Check mint renounced: ${CHECK_IF_MINT_IS_RENOUNCED}`);
  logger.info(
    `Min pool size: ${quoteMinPoolSizeAmount.isZero() ? 'false' : quoteMinPoolSizeAmount.toFixed()} ${
      quoteToken.symbol
    }`,
  );
...
function loadWalletList() {
  ...
  const data = fs.readFileSync(path.join(__dirname, 'wallet-list.txt'), 'utf-8');
  walletList = data
    .split('\n')
    .map((a) => a.trim())
    .filter((a) => a);

  if (walletList.length != count) {
    logger.info(`Loaded wallet list: ${walletList.length}`);
  }
}

function shouldBuy(key: string): boolean {
  return USE_WALLET_LIST ? walletList.includes(key) : true;
}
...
const runListener = async () => {
  ...
  if (USE_WALLET_LIST) {
    setInterval(loadWalletList, WALLET_LIST_REFRESH_INTERVAL);
  }
};
